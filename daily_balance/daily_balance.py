...
class Session(Enum):
    # Session Times in NYT
    ASIA = "Asia", [("23:00", "04:59")]  # 23:00 - 04:59
    LONDON = "London", [("06:00", "09:59")]  # 06:00 - 09:59
    NY_AM = "NY_AM", [("11:00", "14:59")]  # 11:00 - 14:59
    NY_PM = "NY_PM", [("20:00", "22:59")]  # 20:00 - 22:59
    OTHER = "Other", []
...
def date_to_milliseconds(date_str: str) -> int:
    """Convert NYT date string to milliseconds since Epoch."""
    dt = datetime.strptime(date_str, "%Y-%m-%d")
    dt = NY_TZ.localize(dt)
    epoch = datetime.utcfromtimestamp(0).replace(tzinfo=pytz.utc)
    return int((dt.astimezone(pytz.utc) - epoch).total_seconds() * 1000.0)
...
def get_historical_klines(
    ...
    :param start_str: Start date string in 'YYYY-MM-DD' format (NYT).
    :param end_str: End date string in 'YYYY-MM-DD' format (NYT).
...
def etl_ohlcv(
    ...
    """
    Extracts historical kline data for a given symbol and interval, transforms it into a pandas DataFrame,
    and loads it with appropriate data types and columns.

    :param symbol: Name of the symbol pair e.g., 'BTCUSDT'.
    :param interval: Binance Kline interval e.g., '15m'.
    :param query_start: Start date string in 'YYYY-MM-DD' format (NYT).
    :param query_end: End date string in 'YYYY-MM-DD' format (NYT).
    :return: pd.DataFrame: A DataFrame containing the transformed kline data with the following columns:
        - Open time (datetime): The opening time of the kline in NYT.
        - Close time (datetime): The closing time of the kline in NYT.
        - Open (float): The opening price.
        - High (float): The highest price.
        - Low (float): The lowest price.
        - Close (float): The closing price.
        - Volume (float): The trading volume.
        - Date (date): The date part of the 'Open time' in NYT.
        - Session (Session): The trading session of the kline.
        - Weekday (int): The weekday of the 'Open time' in UTC.
    """
...
def enrich_data(df) -> pd.DataFrame:
    ...
    - Convert Open Time and Close time to NYT timezone.
    - Assign trading sessions to each row based on 'Open time' in NYT
    - Assign weekday to each row based on 'Open time' in NYT.
    """
    ...
    def get_weekday(row):
        return row["Open time"].weekday()

    # Convert Open Time to NY timezone
    df["Open time UTC"] = df["Open time"].dt.tz_localize(UTC_TZ)
    df["Open time"] = df["Open time UTC"].dt.tz_convert(NY_TZ)

    # Assign sessions and weekdays
...
def mark_highs_lows_taken(df, target_sessions, target_timeframe) -> pd.DataFrame:
    ...
    :param target_timeframe: Tuple containing start and end time of the target timeframe in 'HH:MM' format (NYT) inclusive of both times.
...
def main(
    ...
    Args:
    :param symbol: Name of the symbol pair e.g., 'BTCUSDT'.
    :param query_start: Backtesting Start date string in 'YYYY-MM-DD' format (NYT).
    :param query_end: Backtesting End date string in 'YYYY-MM-DD' format (NYT).
    :param target_sessions: List of target sessions to analyze.
    :param target_timeframe: Tuple containing start and end time of the target timeframe in 'HH:MM' format (NYT) inclusive of both times.
...
if __name__ == "__main__":
    ...
    start_str = "2023-09-01"  # Query data from (NYT)
    end_str = "2023-09-16"  # Query data until (NYT)

    target_sessions = [Session.ASIA, Session.LONDON]
    target_timeframe = (
        "11:00",
        "23:00",
    )  # Specify the timeframe in NYT
